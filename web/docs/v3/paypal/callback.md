...
如果您没有传参，或传 `null` 则 `yansongda/pay` 会自动识别 PayPal 的回调请求并处理，通过 `Collection` 实例返回 PayPal 的处理参数
...
例如，如果你想在回调的时候使用非默认配置，则可以 `Pay::paypal()->callback(null, ['_config' => 'yansongda'])` 切换为 `yansongda` 这个租户的配置信息。
