...
class QueryStockDetailPlugin implements PluginInterface
{
    ...
    public function assembly(Rocket $rocket, Closure $next): Rocket
    {
        ...
        $rocket->setPayload(array_merge(
            [
                '_method' => 'GET',
                '_url' => 'v3/marketing/favor/stocks/'.$payload->get('stock_id').'?stock_creator_mchid='.$config['mch_id'],
                '_service_url' => 'v3/marketing/favor/stocks/'.$payload->get('stock_id').'?stock_creator_mchid='.$config['mch_id'],
            ],
        ));
