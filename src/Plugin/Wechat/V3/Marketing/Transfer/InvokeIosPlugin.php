...
class InvokeIosPlugin implements PluginInterface
{
    ...
    public function assembly(Rocket $rocket, Closure $next): Rocket
    {
        ...
        if (is_null($packageInfo)) {
            Logger::error('[Wechat][V3][Marketing][Transfer][InvokeIosPlugin] iOS调起用户确认收款失败：响应缺少 `package_info` 参数，请自行检查参数是否符合微信要求', $destination?->all() ?? null);
        ...
        Logger::info('[Wechat][V3][Marketing][Transfer][InvokeIosPlugin] 插件装载完毕', ['rocket' => $rocket]);
