...
/**
 * @see https://pay.weixin.qq.com/doc/v3/merchant/4012647349
 */
class CreatePlugin implements PluginInterface
{
    ...
    {
        Logger::debug('[Wechat][V3][PayScore][Permissions][CreatePlugin] 插件开始装载', ['rocket' => $rocket]);
        ...
        if (Pay::MODE_SERVICE === $config->getMode()) {
            throw new InvalidParamsException(Exception::PARAMS_PLUGIN_ONLY_SUPPORT_NORMAL_MODE, '参数异常: 支付分预授权（签约），只支持普通商户模式，当前配置为服务商模式');
        }

        if (is_null($payload)) {
            throw new InvalidParamsException(Exception::PARAMS_NECESSARY_PARAMS_MISSING, '参数异常: 支付分预授权（签约），参数为空');
        }
        ...
        Logger::info('[Wechat][V3][PayScore][Permissions][CreatePlugin] 插件装载完毕', ['rocket' => $rocket]);
