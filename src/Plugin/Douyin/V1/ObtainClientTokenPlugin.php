...
class ObtainClientTokenPlugin implements PluginInterface
{
    ...
    {
        Logger::debug('[Douyin][V1][ObtainClientTokenPlugin] 插件开始装载', ['rocket' => $rocket]);
        ...
        if (empty($token)) {
            $token = self::getDouyinClientToken($params);
        }

        $rocket->mergePayload(['_access_token' => $token]);

        Logger::info('[Douyin][V1][ObtainClientTokenPlugin] 插件装载完毕', ['rocket' => $rocket]);
