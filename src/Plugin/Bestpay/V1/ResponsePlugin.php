...
class ResponsePlugin implements PluginInterface
{
    ...
    protected function validateResponse(Rocket $rocket): void
    {
        ...
        if ($destination instanceof Collection && true !== $destination->get('success')) {
            throw new InvalidResponseException(Exception::RESPONSE_BUSINESS_CODE_WRONG, sprintf(
                '翼支付返回业务异常: errorCode: %s, errorMsg: %s',
                $destination->get('errorCode'),
                $destination->get('errorMsg')
            ), $destination);
        }
    }
}
